...
Имена экспертов фиксированы; **не придумывай** другие. Генерация **растров** (узлы, итоговый PNG): линейка **GPT Image** через Images API. По умолчанию **`gpt-image-1.5`** — обычно доступна без лишних ограничений; **`gpt-image-2`** качественнее, но у части ключей даёт **403** до [верификации организации](https://platform.openai.com/settings/organization/general). Это **не** «GPT‑2» (старая текстовая модель). Резерв по качеству — **`dall-e-3`** (другие допустимые размеры; код пресета сам подставит **1792×1024** вместо 1536×1024 при финальном PNG).
...
Параметр **`image_model`** у экспертов с картинками: по умолчанию **`gpt-image-1.5`**. Для максимального качества при прошедшей верификации org — **`gpt-image-2`**. Если нужен только DALL·E — **`dall-e-3`**.
...
**Нормальный путь:** один вызов **`floorplan_full_openai_pipeline`** — внутри: Chat → layout/spec → (опц.) изображения узлов через **`image_model`** (по умолчанию `gpt-image-1.5`) → экспорт. Если в **`outputs`** есть **`png`**, по умолчанию (**`final_png_from_openai` = true**) итоговый **`paths.png`** — **один кадр из OpenAI Images** (промпт из нормализованного spec; для GPT Image кадр **1536×1024**, для **dall-e-3** пресет подставит **1792×1024**). Каноническая геометрия — в **`paths.svg`**. Старое поведение PNG из вектора: **`final_png_from_openai` = false**. Отдельный вызов **`floorplan_layout_draft_merge`** при этом **не** нужен.

Зависимость в ExTella: `extella-pip install openai` (актуальный SDK).
